...
from flask import Flask, request, session, g, redirect, \
    url_for, render_template, flash, abort, Response

import shorten_utils

DATABASE = '/var/shorten.db'
DEBUG = False
USERNAME = ''
PASSWORD = ''
...
def add_url(long_url, authenticated = False, return_json=False):
    if not authenticated:
        return redirect('http://www.ilwyw.com', code=302)
    ...
    g.db.execute('insert into entries (long_url, short_url) values (?, ?)', 
        [long_url, short_url])
    g.db.commit()
...
def add_url_via_form():
    if not session.get('logged_in'):
        return redirect('http://ilwyw.com', code=302)
    return add_url(request.form['url'], authenticated=True)

#functions calling the next two view functions 
#have to pass in the url as:
#urllib.quote_plus(json.dumps(unescaped_url))
...
@app.route('/show/')
def show_entries():
    cur = g.db.execute('select long_url, short_url from entries order by id desc')
    entries = [dict(long_url=row[0], short_url=row[1]) for row in cur.fetchall()]
